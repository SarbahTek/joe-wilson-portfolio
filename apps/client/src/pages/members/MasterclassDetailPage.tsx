...
export default function MasterclassDetailPage() {
  ...
  const { data: cohort, isLoading, isError, error } = useMasterclass(cohortId);
  ...
  if (isError) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-white gap-3 px-4">
        ...
        <Link to="/members" className="text-sm text-[#077DA7] hover:underline">Back to Masterclasses</Link>
      </div>
    );
  }
